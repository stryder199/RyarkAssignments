...
dx
d	<a href="ll_add_at_spec">int ll_add_at(struct node** head_ptr,int x,int i)</a>
x	int ll_add_at(struct node** head_ptr,int x,int i)
dx	{
dx		struct node *new,*current;
dx
dx		// if i < 0 or i > n, out of range
dx		if (i < 0 || (*head_ptr == NULL && i > 0)) { $t0
dx			return LL_RANGE_ERROR;
dx		}
dx
dx		// if the list is empty or add the new node at the front
dx		if (*head_ptr == NULL || i == 0) { $t1
...
dx	}
dx
d	<a href="ll_remove_spec">int ll_remove(struct node** head_ptr,int x)</a>
x	int ll_remove(struct node** head_ptr,int x)
dx	{
...
dx	}
dx
d	<a href="ll_find_spec">struct node* ll_find(struct node* head,int x)</a>
x	struct node* ll_find(struct node* head,int x)
dx	{
...
dx	
dx		list_head = (struct node*)malloc(sizeof(struct node));
dx		list_head->x = 1;
dx		list_head->next = (struct node*)malloc(sizeof(struct node));
dx		list_head->next->x = 3;
dx		list_head->next->next = NULL;
dx		ll_add_at(&list_head,5,$x1);
'''
